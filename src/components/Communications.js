...
const Communication = () => {
  ...
  useEffect(() => {
    ...
    const fetchCommunications = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://localhost:8000/api/customers/${selectedCustomerId}/communications`);
  ...
  const handleAddCommunication = async () => {
    try {
      const response = await axios.post(`http://localhost:8000/api/customers/${selectedCustomerId}/communications`, {
        content: newComm,
      });
